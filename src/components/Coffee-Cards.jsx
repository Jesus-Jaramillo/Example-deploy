...
function CardCoffes() {
    ...
    const fetchCoffeeData = async () => {
        try {
            const response = await axios.get(url); // Espera a que axios recupere los datos
            setCoffee(response.data); // Actualiza el estado con los datos recibidos
        } catch (error) {
            console.error("Error fetching coffee data:", error); // Manejo de errores
        }
    };

    useEffect(() => {
        fetchCoffeeData(); // Llama a la función asíncrona en el useEffect
    }, []);
    ...
    return (
        <div>
            ...
            <div className='encabezado'>
                            ...
                            <Card sx={{ maxWidth: 345 }} className='coffee-cards'>
                                ...
                                <div>
                                    <CardContent>
                                        ...
                                        <Typography variant="body2" color="text.secondary">
                                            {coffees.rating ? (
                                                <img src={star_fill} alt="Star_fill" className='controller-star' />
                                            ) : (
                                                <img src={star} alt="Star" className='controller-star' />
                                            )}
                                            <div className='controller-rating'>
                                                <div style={{ color: coffees?.rating ? '#FEF7EE' : '#6F757C' }}>
                                                    {coffees?.rating ? coffees.rating : 'No rating'}
                                                </div>
                                                <div className='controller-votes'>
                                                    {coffees?.votes ? <div>({coffees.votes} votes)</div> : null}
                                                </div>
                                                <div className='controller-available'>
                                                    {coffees.available === false && <div>Sold out</div>}
                                                </div>
                                            </div>
                                        </Typography>
                                    </CardContent>
                                ...
                                <CardActions>
                                    <div>
                                        <Button size="small" onClick={() => handleOpen(coffees)} className='bottom-modal'>
                                            <img src={review} alt="PreReview" height={30} width={30} />
                                        </Button>
                                    </div>
                                </CardActions>
                            </Card>
                ...
                <ModalOpen open={open} handleClose={handleClose} selectedCoffee={selectedCoffees} />
            </div>
        </div>
    );
}
